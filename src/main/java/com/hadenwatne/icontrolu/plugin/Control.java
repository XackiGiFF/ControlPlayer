...
public class Control {
	...
	public Control(iControlU c, Player tar, Player con){
		...
		r = new Random();
		isRunning = false;
		simpleMode = controller.hasMetadata("CP_SimpleMode");
		...
		chat = new PacketAdapter(plugin, ListenerPriority.NORMAL, PacketType.Play.Server.CHAT) {
	        ...
	        public void onPacketSending(PacketEvent event) {
	            if (event.getPlayer().equals(target)) {
	            	if(controller.hasPermission("control.readchat")) {
	...
	public void startControl(){
		...
		if(controlTarget){
			...
			if(target.getHealth() <= controller.getHealthScale())
				controller.setHealth(target.getHealth());
			else controller.setHealth(20d);
			
			if(!simpleMode)
				if(!controller.hasPermission("control.notimelimit")){
	...
	public void stopControl(){
		...
		if(!simpleMode)
			if(!controller.hasPermission("control.nocooldown"))
